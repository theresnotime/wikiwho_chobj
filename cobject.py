...
class ChangeObject:
    ...
    def save_hd5(self, save_dir):
        ...
        change_objects = []
        self.wiki.revisions.iloc[:-1].apply(lambda revision: change_objects.append(revision.change_df))

        timestamp_s = pd.to_datetime([ rev.timestamp for rev in  self.wiki.revisions.values.ravel().tolist()])
        ...
        change_dataframe_path = os.path.join(save_dir, self.article_name+"_change.h5")
